...
class ChangeDateSub extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        "CREATE FUNCTION `enerji`.`device_date_sub`(`device_id` int,`data_id` int,`start_time` datetime,`end_time` datetime) RETURNS decimal(20,2)
        BEGIN
        
        if isnull(end_time) or  end_time < start_time then set end_time = now();END IF;
            set @r_value = 0;
            set @s_value = 0 ;
            set @e_value = 0;
