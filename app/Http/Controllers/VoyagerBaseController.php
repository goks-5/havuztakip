...
class VoyagerBaseController extends Base
{
    ...
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if (in_array($dataType->model_name, $this->optionsModels)) {
                $query->where('company_id', '=', Auth::user()->company_id);
    ...
    public function remove_media(Request $request)
    {
        try {
            ...
            if (isset($fileToRemove['download_link'])) {
                $fileToRemove = $fileToRemove['download_link'];
            }
    ...
    public function relation(Request $request)
    {
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                if (in_array($options->model, $this->companymodels)) {
                    $relationshipOptions =  $relationshipOptions->where('company_id', '=', Auth::user()->company_id)->get();
                } elseif (in_array($dataType->model_name, $this->dashboardmodels)) {
                    $relationshipOptions =  $relationshipOptions->where('user_id', '=', Auth::user()->id)->get();
                } elseif (in_array($dataType->model_name, $this->optionsModels)) {
                    $relationshipOptions =  $relationshipOptions->where('company_id', '=', Auth::user()->company_id)
