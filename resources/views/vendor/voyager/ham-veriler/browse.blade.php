...
@section('page_title', __('voyager::generic.viewing') . ' ' . $dataType->getTranslatedAttribute('display_name_plural'))
    ...
    @foreach ($devices as $device)
        @if ($device->id == $search->device_id)
            @php
                $deviceName = $device->name;
                $tags = (array)json_decode($device->tags); 
                $tagName = $tags[$search->data_id];
            @endphp
        @endif
    @endforeach
...
@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            <i class="{{ $dataType->icon }}"></i>   {{ $deviceName }} - {{  $tagName }}
        </h1>
        @can('add', app($dataType->model_name))
            <a href="{{ route('voyager.' . $dataType->slug . '.create') }}?device_id={{ $search->device_id }}&data_id={{ $search->data_id }}"
        ...
        @can('delete', app($dataType->model_name))
            @if ($usesSoftDeletes)
                <input type="checkbox" @if ($showSoftDeleted) checked @endif
                    id="show_soft_deletes" data-toggle="toggle" data-on="{{ __('voyager::bread.soft_deletes_off') }}"
                            ...
                            <form method="get" class="form-search">
                                <div id="search-input">
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <select class="form-control select2" id="device" name="device_id">
                                            @foreach ($devices as $device)
                                                <option value='{{ $device->id }}' @if ($device->id == $search->device_id) selected @endif>
                                                    {{ $device->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select class="form-control select2" name="data_id" id="deviceTags">
                                        </select>
                                    </div>
                                    <div class="col-md-3">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#device').change(function() {
                ...
                device_index = {{ $search->data_id ?? 0 }};
                selected = $(this).val();
                obj = devices[selected];
                $('#deviceTags').empty();
                Object.keys(obj).forEach(function(k) {
                    if (device_id == selected && device_index == k) {
                        $('#deviceTags').append("<option value='" + k + "' selected>" + obj[k] +
                            "</option>");
                    } else {
                        $('#deviceTags').append("<option value='" + k + "'>" + obj[k] +
                        "</option>");
                    }
                });
            });
