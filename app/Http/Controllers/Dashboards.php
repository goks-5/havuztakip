...
class Dashboards extends VoyagerBaseController
{
  ...
  {
    $sourceDashboard = Dashboard::find($source);

    $targetDashboard = Dashboard::where('user_id', Auth::user()->id)
      ->where('company_id', Auth::user()->company_id)
      ->where('id', $target)
      ->first();
    ...
    if ($sourceDashboard && $targetDashboard) {
      ...
      $sourceTools = DashboardTool::where('dashboard_id', $sourceDashboard->id)
        ...
        foreach($sourceTools as $tool){
            $newTool = new DashboardTool();
            $newTool->dashboard_id = $targetDashboard->id;
            $newTool->row_id = $tool->row_id;
            $newTool->row_index = $tool->row_index;
            $newTool->order = $tool->order;
            $newTool->title = $tool->title;
            $newTool->type = $tool->type;
            $newTool->settings = $tool->settings;
            $newTool->style = $tool->style;
            $newTool->save();
          }
          return "sucsess";
    }
