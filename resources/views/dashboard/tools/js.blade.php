<script type="text/javascript">
  $(document).ready(function() {
    ...
    function gorev() {
      ...
      $(function() {
        $( ".resizable" ).resizable({grid:10 ,stop: function( event, ui ) {postsize(ui);  }});
        $( ".resizable" ).draggable({ snap: true, scroll: false,stop: function( event, ui ) {postsize(ui);  }});
  ...
  $('.boardlink').blur(function() {
    $.ajax({
      ...
      type: 'post',
      data: {id:$(this).data('id'),title:$(this).text(),action_type:'changeBoardName'},
      });
  });
    ...
    $.ajax({
      url: '{{route('toolStyle')}}',
      type: 'post',
      data: {id:ui.helper.data('tool'),style:ui.helper.attr("style")},
      });
    gorev();
    }
    ...
    function DeviceIndicator(data) {
      Object.keys(data).forEach(function(k) {
        if ($('#indicator_out_' + data[k].tool).length) {
          $('#indicator_out_' + data[k].tool).css("box-shadow", "0px 0px 8px 1px " + data[k].color);
      ...
      Object.keys(data).forEach(function(k) {
        ...
        var a = document.getElementById('audio_' + data[k].tool);
          ...
          if(data[k].alarm == true){
            $('#indicator_in_' + data[k].tool).css("background-image", "url(/storage/widget/alarm.gif)");
            if(data[k].sound == '1'){
              a.play();
            }
          }else{
            $('#indicator_in_' + data[k].tool).css("background-image", "url(/storage/widget/alarmoff.png)");
            if(data[k].sound == '1'){
              a.pause();
            }
          }
...
Object.keys(data[k][s]['tags']).forEach(function(ta){
  if(data[k][s]['end_time'] == null){
    activeDevice[data[k][s]['id']] = allDevice[data[k][s]['tags_group']];
    data[k][s]['end_time'] = ' <a href="#" class="btn btn-danger btn-sm"  style="float: right;" data-href="{{route('end_tag')}}?id='+  data[k][s]['id'] + '"  data-toggle="modal" data-target="#confirm-end">Bitir</a>';
  }
t.row.add( [
        data[k][s]['name'] + ' ' + data[k][s]['start_time'] + ' - ' + data[k][s]['end_time']  ,
...
t.row.add(rows);
  ...
  */
    ...
    function fswitch(data) {
      Object.keys(data).forEach(function(k) {
        if ($('#switch_' + k).length) {
           if($('#switch_' + k).data('onvalue') == data[k]  && !$('#switch_' + k).prop("checked")){
            $('#switch_' + k).bootstrapToggle('on');
           }else if($('#switch_' + k).data('offvalue') == data[k]  && $('#switch_' + k).prop("checked")){
